...
public class MultyPlayerControll : MonoBehaviourPun
{
    [SerializeField] MultyGrapplingRope m_GrapplingRope;
    [Header("Camera")]
    ...
    public MultyGrapplingGun Grapp;
    ...
    void Start()
    {
        ...
        m_CameraShake = GameObject.Find("SoundMng").GetComponent<CameraShake>();
    }
    enum PlayerState
    {
        Nomal,
    ...
    public void PlayerSkill()
    {
        ...
        if (GameMng.GetIns.SkillCoolTime < 0 && Input.GetKeyDown(KeyCode.E))
        {
            ...
            if (GameMng.GetIns.PlayerAttackMode == 1)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    GameObject b = PhotonNetwork.Instantiate("Prefabs/Mkunai", dir, Quaternion.identity);
                    int y = look ? 180 : 0;
                    b.GetComponent<MThrow>().MyQuaternion(new Vector3(0, y, (i * 15) + 150));
                    Destroy(b, 10f);
                }
            }
